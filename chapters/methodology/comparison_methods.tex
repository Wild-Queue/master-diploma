...
\[
    I_{dec}(L) =
    \begin{cases}
        1, & \text{if the language allows explicit subtyping without inheritance constructs} \\
        0.5, & \text{if such separation exists but is restricted (e.g., limited to specific constructs)} \\
        0, & \text{if subtyping is defined only through inheritance}
    \end{cases}
\]
...
\[
    I_{ind}(L) =
    \begin{cases}
        1, & \text{if subtyping rules are defined independently of inheritance semantics} \\
        0.5, & \text{if independence is partial or implicit} \\
        0, & \text{if subtyping is fully derived from inheritance}
    \end{cases}
\]
...
\[
    I_{form}(L) =
    \begin{cases}
        1, & \text{if inheritance and subtyping are formally specified as distinct relations} \\
        0.5, & \text{if separation is stated but не полностью формализована} \\
        0, & \text{if distinction отсутствует или не определена в спецификации}
    \end{cases}
\]
...
\begin{itemize}
    ...
    N_{impl}(L) = 
    \begin{cases}
        1, & \text{if number of interfaces is not restricted}, \\
        0.66, & \text{if the number of interfaces is limited, but it is greater than 1}, \\
        0.33, & \text{if the number of interfaces is limited by 1 interface}, \\
        0, & \text{if there are no interfaces or their analogues}.
    \end{cases}
    \]
...
\begin{itemize}
    ...
    \item Weights are set as $w_{single}=1$, $w_{multi}=2$, $w_{pred}=2$
        \begin{itemize}
            \item Single dispatch weight is $1$ since it often requires structural workarounds. 
...
\[I_{static}(L) =
    \begin{cases}
        1, & \text{if the language guarantees dispatch safety at compile time} \\
        0, & \text{otherwise}
    \end{cases}
\]
...
\[I_{exhaust}(L) =
    \begin{cases}
    1, & \text{if the language enforces exhaustive dispatch definitions} \\
    0, & \text{otherwise}
    \end{cases}
\]
