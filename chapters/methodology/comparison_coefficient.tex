...
The choice of weighting coefficients for the proposed metrics is guided by the main objective of this work: to emphasize those aspects of object models that most significantly affect extensibility, safety and semantic clarity in object-oriented programming. The coefficients are calibrated to prioritize semantic soundness over syntactic convenience, explicitness over implicit behaviour, and composability over rigid hierarchy structures. In addition, separation of concerns-particularly between subtyping, inheritance and composition-is critical for scalable software architecture \cite{odersky2004overview, cook1989inheritance}. The higher weights are systematically assigned to components reflecting formal guarantees and semantic orthogonality.

\subsection{Type System and Subtyping Model}

We determine the weighting coefficients for the TSSM metric based on the importance of each component for comparing object models across modern programming languages. The higher weights is given to characteristics that have a stronger impact on object-oriented software development.

\[
    \alpha = 0.25, \beta = 0.3, \gamma = 0.45
\]

The highest weight is assigned to the inheritance-subtyping separation component ($\gamma = 0.45$).  This decision is justified by a literature that identifies the conflation of inheritance and subtyping as a major flaw in mainstream object-oriented languages \cite{cook1989inheritance}. Failure to distinguish them leads to violations of the Liskov Substitution Principle and undermines sound modular reasoning. Liskov and Wing further formalize this issue, noting that subtype relationships must preserve behavioural specifications independently of implementation structure \cite{liskov1999behavioral}. This separation is treated as the most critical factor influencing the overall soundness of the object model.

The variance formalization component is assigned the second-highest weight ($\beta = 0.30$), as it directly impacts type safety in the presence of parametric polymorphism  \cite{pierce2002types}. Historical problems in Java generics, such as the need for use-site variance (wildcards), exemplify the complexity introduced by variance systems that are not expressive enough or only partially formalized.  

The balance component of the nominal-structural typing trade-off is assigned a slightly lower weight ($\alpha = 0.25$). While the importance of this component for flexibility and expressiveness is clear, its impact on soundness is more indirect than the other components. Cardelli and Wegner argue that no single typing discipline is sufficient for all types of abstraction \cite{cardelli1985understanding}, which motivates hybrid approaches that combine nominal and structural typing. Structural typing improves composability and reduces coupling, while nominal typing improves readability and explicitness of design intent. Hence, $\alpha$ is assigned a moderate weight, reflecting its role as an enabler of expressive type design rather than a primary determinant of soundness.

\subsection{Abstraction and Encapsulation Mechanisms}

The weighting coefficients of the AE metric are defined by the importance of each component for object model comparison in modern programming languages. Characteristics that have a greater effect on object-oriented software development are assigned higher weights
...
The interface abstraction capacity component is also assigned $\gamma = 0.25$, as the ability to separate specification from implementation is fundamental to object-oriented design. Liskov and Wing emphasise that abstraction mechanisms must support behavioural subtyping independently of implementation details \cite{liskov1999behavioral}. Languages that offer several abstraction constructs (interfaces, abstract classes, traits, protocols) allow more flexible and precise contract modelling.

The visibility granularity index receives the lowest weight ($\alpha = 0.20$), since fine-grained access modifiers help encapsulation but they are not enough by themselves. Multiple visibility levels do not prevent violations if the language supports reflection or unsafe access.

\subsection{Inheritance Semantics and Hierarchy Management}

We derive the weighting coefficients for the ISRI metric from the importance of each component for the comparison of object models in modern programming languages. We assign higher weights to characteristics with a stronger influence on object-oriented software development.
...
We determine the weighting coefficients for the CRFI metric based on the importance of each component for comparing object models across modern programming languages. We assign higher weights to characteristics that exert a stronger influence on object-oriented software development.
...
We determine the weighting coefficients for the ORCI metric based on the importance of each component for comparing object models across modern programming languages. We assign higher weights to characteristics that exert a stronger influence on object-oriented software development.
...
The identity semantics score ($\alpha = 0.35$) is critical because object identity is a defining property of object-oriented programming. As emphasized by Abadi and Cardelli, objects are characterized by identity, state, and behaviour \cite{abadi2012theory}. Inconsistent treatment of identity across user-defined types leads to semantic fragmentation, where some objects behave as values while others behave as references. This inconsistency complicates reasoning about aliasing, mutability, and side effects.

The meta-level extensibility score ($\gamma = 0.35$) is equally significant, as it captures the ability of a language to support reflection, intercession, and customization of object behaviour. Metaobject protocols (MOPs), as described by Kiczales et al., enable programmers to adapt the language implementation to suit the needs of specific applications \cite{kiczales1991art}. This includes modifying object layout, method dispatch, and instantiation semantics.
...
We determine the weighting coefficients for the PDEI metric based on the importance of each component for comparing object models across modern programming languages. We assign higher weights to characteristics that exert a stronger influence on object-oriented software development.

\[
\alpha = 0.35, \beta = 0.35, \gamma = 0.30
\]

The highest weights are assigned to dispatch scope and dispatch safety ($\alpha = 0.35$, $\beta = 0.35$), as these components fundamentally determine both the expressive capabilities and the semantic reliability of polymorphic behaviour.
...
We determine the weighting coefficients for the FSVI metric based on the importance of each component for comparing object models across modern programming languages. We assign higher weights to characteristics that exert a stronger influence on object-oriented software development.
