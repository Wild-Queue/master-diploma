...
\begin{aligned}
    ...
    = 0.3 \cdot 0.5 + 0.3 \cdot 1 + 0.4 \cdot \frac{5}{6} \\
    \approx 0.7833 
\end{aligned}
\]
...
\begin{aligned}
    ...
    = 0.2 \cdot 1 + 0.25 \cdot 0 + 0.25 \cdot 0.75 + 0.3 \cdot 0 \\
    = 0.3875
\end{aligned}
\]
...
\begin{aligned}
    ...
    = 0.3 \cdot 0.5 + 0.35 \cdot 0 + 0.35 \cdot 1 \\
    = 0.5
\end{aligned}
\]
...
\begin{aligned}
    ...
    = 0.35 \cdot 0.6 + 0.3 \cdot 1 + 0.35 \cdot \frac{2}{3} \\
    \approx 0.7433
\end{aligned}
\]
...
The language supports a form of predicate-based dispatch through pattern matching constructs (e.g., \texttt{switch} expressions with guards and \texttt{is}-patterns), which enable branching based on both type and additional logical conditions.
...
\begin{aligned}
    ...
    = 0.35 \cdot 0.6 + 0.35 \cdot 0.5 + 0.3 \cdot 1 \\
    = 0.685
\end{aligned}
\]
...
\begin{aligned}
    ...
    = 0.35 \cdot 0 + 0.25 \cdot 0.6 + 0.4 \cdot 0.25 \\
    = 0.25
\end{aligned}
\]
...
\[
\begin{aligned}
    TSSI(\text{Java}) = 0.3 \cdot S_{nom}(\text{Java}) + 0.3 \cdot S_{var}(\text{Java}) + 0.4 \cdot S_{rel}(\text{Java}) \\
    = 0.3 \cdot 0.5 + 0.3 \cdot 0.5 + 0.4 \cdot 0.5 \\
    = 0.5
\end{aligned}
\]
...
\[
\begin{aligned}
    ISRI(\text{Java}) = 0.2 \cdot H_s(\text{Java}) + 0.25 \cdot L_d(\text{Java}) + 0.3 \cdot C_p(\text{Java}) + 0.25 \cdot F_b(\text{Java}) \\
    = 0.2 \cdot \frac{4}{7} + 0.25 \cdot 1 + 0.3 \cdot \frac{4}{6} + 0.25 \cdot \frac{4}{6} \\
    \approx 0.731
\end{aligned}
\]
...
Java does not provide native syntactic delegation keywords (such as \texttt{delegate} in other languages). The delegation is typically implemented manually via composition and adapter patterns, where methods explicitly forward calls to a contained object. 
...
\[
\begin{aligned}
    K_{del}(\text{Java}) = 0.5, S_{fwd}(\text{Java}) = 0 \\
    \Rightarrow D_{cls}(\text{Java}) = \frac{0.5 + 0}{2} = 0.25
\end{aligned}
\]

\textbf{2) Trait Expressiveness Score $T_{exp}(L)$}

In the presence of multiple inherited default methods with the same signature, Java does not support conflict resolution via aliasing or exclusion. It enforces a compile-time error unless the implementing class explicitly overrides the conflicting method and resolves the ambiguity (JLS \S8.4.8, \S9.4.1.3).
...
\[
\begin{aligned}
    R_{conf}(\text{Java}) = 0, O_{mod}(\text{Java}) = 0 \\
    \Rightarrow T_{exp}(\text{Java}) = 0
\end{aligned}
\]
...
\[
\begin{aligned}
    N_{impl}(\text{Java}) = 1, D_{def}(\text{Java}) = 1 \\
    \Rightarrow I_{comp}(\text{Java}) = 1
\end{aligned}
\]
...
\[
\begin{aligned}
    ORCI(\text{Java}) = 0.35 \cdot S_{id}(\text{Java}) + 0.3 \cdot S_{cr}(\text{Java}) + 0.35 \cdot S_{meta}(\text{Java}) \\
    = 0.35 \cdot 1 + 0.3 \cdot \frac{3}{4} + 0.35 \cdot \frac{1}{3} \\
    \approx 0.6917
\end{aligned}
\]
...
\[
\begin{aligned}
    PDEI(\text{Java}) = 0.35 \cdot M_{scope}(\text{Java}) + 0.35 \cdot S_{disp}(\text{Java}) + 0.3 \cdot O_{virt}(\text{Java}) \\
    = 0.35 \cdot 0.2 + 0.35 \cdot 0.5 + 0.3 \cdot 1 \\
    = 0.545
\end{aligned}
\]
...
Next, we identify constructs that impose compile-time constraints on reference usage: the \texttt{final} keyword, which restricts reassignment of references, generic type bounds (e.g., \texttt{<T extends ...>}), which constrain admissible reference types, and access modifiers (e.g., \texttt{private}, \texttt{protected}) that statically restrict reference visibility and usage contexts.
...
\textbf{Final metric value:}
\[
    SCSI(\text{Java}) = 0.35 \cdot M_{def}(\text{Java}) + 0.25 \cdot S_{dens}(\text{Java}) + 0.4 \cdot P_{conc}(\text{Java})
    = 0.35 \cdot 0 + 0.25 \cdot 0.6 + 0.4 \cdot 0.25
    = 0.25
\]
...
\[
\begin{aligned}
    FSVI(\text{Java}) = 0.45 \cdot C_{nat}(\text{Java}) + 0.55 \cdot V_{enf}(\text{Java}) \\
    = 0.45 \cdot 0 + 0.55 \cdot 0.5 \\
    = 0.275
\end{aligned}
\]
...
\[
\begin{aligned}
    AE(\text{Smalltalk}) = 0.2 \cdot V(\text{Smalltalk}) + 0.25 \cdot M(\text{Smalltalk}) + 0.25 \cdot I(\text{Smalltalk}) + 0.3 \cdot E(\text{Smalltalk}) \\
    = 0.2 \cdot \frac{1}{7} + 0.25 \cdot 0 + 0.25 \cdot 0 + 0.3 \cdot 0 \\
    \approx 0.0286
\end{aligned}
\]
...
\[
\begin{aligned}
    ISRI(\text{Smalltalk}) = 0.2 \cdot H_s(\text{Smalltalk}) + 0.25 \cdot L_d(\text{Smalltalk}) + 0.3 \cdot C_p(\text{Smalltalk}) + 0.25 \cdot F_b(\text{Smalltalk}) \\
    = 0.2 \cdot 1 + 0.25 \cdot 1 + 0.3 \cdot 0 + 0.25 \cdot 0 \\
    = 0.45
\end{aligned}
\]
...
\[
\begin{aligned}
    CRFI(\text{Smalltalk}) = 0.3 \cdot D_{cls}(\text{Smalltalk}) + 0.35 \cdot T_{exp}(\text{Smalltalk}) + 0.35 \cdot I_{comp}(\text{Smalltalk}) \\
    = 0.3 \cdot 0.75 + 0.35 \cdot 0 + 0.35 \cdot 0 \\
    = 0.225
\end{aligned}
\]
...
\[
\begin{aligned}
    I_{level}(\text{Smalltalk})=3, I_{level}(\text{JS/Python/Ruby}) = 3 \\
    \Rightarrow S_{meta}(\text{Smalltalk})= \frac{1}{3}
\end{aligned}
\]
...
\[
\begin{aligned}
    ORCI(\text{Smalltalk}) = 0.35 \cdot S_{id}(\text{Smalltalk}) + 0.3 \cdot S_{cr}(\text{Smalltalk}) + 0.35 \cdot S_{meta}(\text{Smalltalk}) \\
    = 0.35 \cdot 1 + 0.3 \cdot \frac{1}{4} + 0.35 \cdot \frac{1}{3} \\
    \approx 0.5417
\end{aligned}
\]
...
\[
\begin{aligned}
    PDEI(\text{Smalltalk}) = 0.35 \cdot M_{scope}(\text{Smalltalk}) + 0.35 \cdot S_{disp}(\text{Smalltalk}) + 0.3 \cdot O_{virt}(\text{Smalltalk}) \\
...
\[
\begin{aligned}
    SCSI(\text{Smalltalk}) = 0.35 \cdot M_{def}(\text{Smalltalk}) + 0.25 \cdot S_{dens}(\text{Smalltalk}) + 0.4 \cdot P_{conc}(\text{Smalltalk}) \\
...
\[
\begin{aligned}
    FSVI(\text{Smalltalk}) = 0.45 \cdot C_{nat}(\text{Smalltalk}) + 0.55 \cdot V_{enf}(\text{Smalltalk}) \\
