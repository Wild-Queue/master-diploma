...
Scala does not provide built-in syntactic delegation constructs (i.e., no dedicated keyword such as \texttt{delegate}). The delegation is typically emulated via composition and forwarding methods or via library-level patterns. 
...
$n_{pre}(\text{Scala}) = 0$: Scala does not define native syntax for preconditions. Although developers may use \texttt{require} from the standard library, it is a conventional method rather than a first-class language construct with formal semantic enforcement.

$n_{post}(\text{Scala}) = 0$: Scala lacks intrinsic postcondition constructs (unlike languages with \texttt{ensures} clauses). While \texttt{ensuring} exists as a library extension method, it is not specified as a core semantic contract mechanism.
...
There are no formally defined variance rules, nor any specification-level guarantees of type soundness related to variance. The type behavior is governed entirely by runtime semantics without static verification or proofs of safety. 
...
JavaScript provides ES modules as the primary modularization construct, where bindings are explicitly exported and imported via \texttt{export} and \texttt{import} declarations, and only exported identifiers are accessible outside the module. These constitute strict boundaries with enforced export control. Additionally, other constructs such as script scope or function scope do not qualify as module systems in the specification-level sense of namespace isolation.
...
However, regarding soundness, Python’s type system is not enforced at runtime and relies on external static type checkers (e.g., mypy), and the official specification does not provide a fully formalized, proven type-safe variance model. It imposes practical restrictions to mitigate unsoundness. 
...
Third, Python does not provide first-class invariant constructs at the language level, such behavior must be manually encoded or delegated to third-party libraries.
...
$p_3=0$: There is no requirement for explicit \texttt{override} annotations.
...
$s_1=0$: Ruby does not require explicit \texttt{override} annotations. 

$s_2=1$: However, it provides method visibility modifiers (\texttt{public}, \texttt{protected}, \texttt{private}), which partially restrict unintended access and override scope. 
